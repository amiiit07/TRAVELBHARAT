...
function SearchContent() {
  ...
  useEffect(() => { if (q) api.search(q).then(setResults).catch(console.error).finally(() => setLoading(false)); }, [q]);

  if (!q) return <div className="min-h-screen pt-24 flex items-center justify-center"><p className="text-white/60">Enter a search term</p></div>;
    ...
    <div className="min-h-screen pt-24 pb-12">
      <div className="max-w-7xl mx-auto px-4">
        <motion.div initial={{opacity:0}} animate={{opacity:1}} className="mb-8">
          ...
          <p className="text-white/60">Showing results for "{q}"</p>
        </motion.div>
        {loading ? <div className="space-y-4">{[1,2,3].map(i => <div key={i} className="h-20 bg-surface rounded-xl animate-pulse"/>)}</div> : (
          <>
